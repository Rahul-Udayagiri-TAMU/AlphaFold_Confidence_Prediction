...
def maybe_standardize(model_name, X_train, X_val, X_test):
    """
    Scale for models that usually benefit from it.
    """
    need_scaling = model_name in {"ridge", "svr", "mlp"}
...
def main():
    ...
    parser.add_argument("--model", type=str, required=True, choices=["ridge", "svr", "rf", "xgb", "mlp"])
    ...
    X_train_p, X_val_p, X_test_p, scaler = maybe_standardize(
        args.model, X_train, X_val, X_test
    )
